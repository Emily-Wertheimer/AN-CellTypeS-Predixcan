...
colnames(sumstats_merge_GTEx)[4] <- 'noneffect_allele'
colnames(sumstats_merge_GTEx)[5] <- 'effect_allele'
colnames(sumstats_merge_GTEx)[11] <- 'p'
colnames(sumstats_merge_GTEx)[22] <- 'variant.id.hg38'
colnames(sumstats_merge_GTEx)[23] <- 'rs_id_dbSNP151_GRCh38p7'
colnames(sumstats_merge_GTEx)[10] <- 'effect_error'

dim(sumstats_merge_GTEx) #dim: 6804937      23


######################################## 

sumstats_save_filt <- sumstats_merge_GTEx # %>% dplyr::rename(effect_allele='alt', noneffect_allele='ref')

#Save new columns
sumstats_save_final <- sumstats_save_filt %>% select(all_of(c('rs_id_dbSNP151_GRCh38p7','effect_allele','noneffect_allele','p','OR')))
...
freeze3.gwas.sumstats.clean2 <- sumstats_save_final
...
write.table(freeze3.gwas.sumstats.clean2, file="/home/ekw28/AN-CellTypeS-Predixcan/freeze3.gwas.sumstats.clean2", sep = "\t", col.names=T, row.names = F,quote=F)
